...
const nthTodoCheckbox = (nth) =>
  locate("div > input").inside(`.todo-list li:nth-child(${nth})`);
const nthTTodoDeleteButton = (nth) =>
  locate("div > button")
    .inside(`.todo-list li:nth-child(${nth})`)
    .as(`${nth}nth delete button`);
const nthTodoEditField = (nth) =>
  locate("form > input")
    .inside(`.todo-list li:nth-child(${nth})`)
    .as(`${nth}nth todo input`);
...
module.exports = {
  ...
  async markNthAsCompleted(nthTodo) {
    ...
    assert(
      classNames.indexOf("completed") < 0,
      "Expected todo to be not already marked as completed"
    );
